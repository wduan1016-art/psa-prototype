    ...
    <div className={`prototype-shell ${collapsed ? "is-collapsed" : ""}`}>
      ...
      <div className="app-main">
        <header className="app-header">
          ...
          <div className="header-actions">
            ...
            <PopoverButton label="快捷新建" icon="＋" open={quickOpen} onToggle={() => setQuickOpen(value => !value)}>
              <Popover title="快捷新建"><QuickItem icon="◎" label="新建商机" note="进入商机管理" onClick={() => navigate("opportunity-list")} /><QuickItem icon="▣" label="新建项目" note="进入项目管理" onClick={() => navigate("project-list")} /><QuickItem icon="☷" label="新建任务" note="进入第 4 项设计" onClick={() => navigate("all-tasks")} /></Popover>
            </PopoverButton>
        ...
        <main className="page-canvas">
          {page === "home" ? <PersonalWorkspace onNavigate={navigate} /> : page === "my-todos" ? <TodoPage onNavigate={navigate} /> : page === "notifications" ? <NotificationPage /> : page === "opportunity-list" || page === "opportunity-board" ? <OpportunityManagement view={page === "opportunity-board" ? "board" : "list"} onNavigate={navigate} /> : page === "project-list" || page === "project-board" ? <ProjectManagement view={page === "project-board" ? "board" : "list"} onNavigate={navigate} /> : <ModulePlaceholder page={page} />}
        </main>
      </div>
    </div>
  );
...
type Project = { id:string; name:string; customer:string; source:string; stage:string; mic:string; pic:string; progress:number; health:"normal"|"warning"|"danger"; start:string; end:string; amount:number; team:number; tasks:string; milestone:string };
const projectStages = ["项目启动","规划设计","实施交付","验收结项"];
const projects: Project[] = [
  {id:"PRJ-2026-003",name:"智能研发协同平台",customer:"吉利研究院",source:"商机转化",stage:"实施交付",mic:"王迪",pic:"李娜",progress:68,health:"normal",start:"2026-04-08",end:"2026-09-30",amount:280,team:8,tasks:"36 / 52",milestone:"用户验收测试"},
  {id:"PRJ-2026-006",name:"供应链流程优化",customer:"恒星零售",source:"商机转化",stage:"验收结项",mic:"李娜",pic:"赵倩",progress:92,health:"warning",start:"2026-02-16",end:"2026-07-31",amount:210,team:5,tasks:"43 / 46",milestone:"项目终验"},
  {id:"PRJ-2026-009",name:"DevSecOps 能力建设",customer:"远海金融",source:"直接新建",stage:"规划设计",mic:"周晨",pic:"王迪",progress:34,health:"normal",start:"2026-06-01",end:"2026-11-15",amount:165,team:6,tasks:"12 / 38",milestone:"总体方案评审"},
  {id:"PRJ-2026-011",name:"数据治理一期",customer:"浦江能源",source:"商机转化",stage:"项目启动",mic:"赵倩",pic:"周晨",progress:12,health:"danger",start:"2026-07-01",end:"2026-12-20",amount:420,team:7,tasks:"4 / 34",milestone:"启动会"},
  {id:"PRJ-2026-012",name:"客户数据中台升级",customer:"北辰医药",source:"直接新建",stage:"实施交付",mic:"王迪",pic:"赵倩",progress:57,health:"warning",start:"2026-03-18",end:"2026-10-15",amount:320,team:9,tasks:"28 / 49",milestone:"数据迁移演练"},
];

function ProjectManagement({view,onNavigate}:{view:"list"|"board";onNavigate:(page:PageId)=>void}) {
  const [selected,setSelected]=useState<Project|null>(null); const [creating,setCreating]=useState(false); const [keywordInput,setKeywordInput]=useState(""); const [keyword,setKeyword]=useState(""); const [stageInput,setStageInput]=useState("全部阶段"); const [stageFilter,setStageFilter]=useState("全部阶段"); const [micInput,setMicInput]=useState("全部 MIC"); const [micFilter,setMicFilter]=useState("全部 MIC"); const [picInput,setPicInput]=useState("全部 PIC"); const [picFilter,setPicFilter]=useState("全部 PIC"); const [toast,setToast]=useState("");
  const filtered=projects.filter(item=>(stageFilter==="全部阶段"||item.stage===stageFilter)&&(micFilter==="全部 MIC"||item.mic===micFilter)&&(picFilter==="全部 PIC"||item.pic===picFilter)&&`${item.name}${item.customer}${item.id}`.includes(keyword));
  const query=()=>{setKeyword(keywordInput.trim());setStageFilter(stageInput);setMicFilter(micInput);setPicFilter(picInput)}; const notify=(message:string)=>{setToast(message);window.setTimeout(()=>setToast(""),2200)};
  return <div className="standard-page project-page"><header className="opportunity-heading"><div><p>PSA / 项目管理</p><h1>{view==="list"?"项目列表":"项目看板"}</h1><span>管理项目从启动、规划、交付到验收结项的完整生命周期。</span></div><div className="page-actions"><button className="secondary-action" onClick={()=>notify("项目数据已刷新")}>↻ 刷新</button><button className="primary-action" onClick={()=>setCreating(true)}>＋ 新建项目</button></div></header>
    <section className="opportunity-kpis"><OpportunityKpi label="进行中项目" value="12" note="本月新启动 3 个" tone="blue"/><OpportunityKpi label="合同总额" value="¥ 3,860万" note="已确认收入 ¥2,170万" tone="violet"/><OpportunityKpi label="本月待验收" value="3" note="其中 1 个存在风险" tone="green"/><OpportunityKpi label="风险项目" value="4" note="1 个高风险 · 3 个关注" tone="amber"/></section>
    <section className="opportunity-toolbar"><div className="view-switch"><button className={view==="list"?"active":""} onClick={()=>onNavigate("project-list")}>☷ 列表</button><button className={view==="board"?"active":""} onClick={()=>onNavigate("project-board")}>▦ 看板</button></div><div className="opportunity-search"><span>⌕</span><input value={keywordInput} onChange={e=>setKeywordInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&query()} placeholder="搜索项目名称、客户或编号"/></div><select aria-label="项目阶段" value={stageInput} onChange={e=>setStageInput(e.target.value)}><option>全部阶段</option>{projectStages.map(stage=><option key={stage}>{stage}</option>)}</select><select aria-label="MIC" value={micInput} onChange={e=>setMicInput(e.target.value)}><option>全部 MIC</option>{Array.from(new Set(projects.map(item=>item.mic))).map(mic=><option key={mic}>{mic}</option>)}</select><select aria-label="PIC" value={picInput} onChange={e=>setPicInput(e.target.value)}><option>全部 PIC</option>{Array.from(new Set(projects.map(item=>item.pic))).map(pic=><option key={pic}>{pic}</option>)}</select><button className="opportunity-query" onClick={query}>查询</button></section>
    {view==="list"?<ProjectTable items={filtered} onSelect={setSelected}/>:<ProjectBoard items={filtered} onSelect={setSelected}/>} {selected&&<ProjectDetail item={selected} onClose={()=>setSelected(null)} onNotify={notify}/>} {creating&&<ProjectCreate onClose={()=>setCreating(false)} onCreated={()=>{setCreating(false);notify("项目已创建，当前为原型模拟数据")}}/>}{toast&&<div className="prototype-toast">✓ {toast}</div>}</div>;
}
function ProjectTable({items,onSelect}:{items:Project[];onSelect:(item:Project)=>void}) {return <section className="project-table"><div className="project-table-head"><span>项目信息</span><span>阶段</span><span>进度</span><span>健康度</span><span>MIC</span><span>PIC</span><span>任务</span><span>计划周期</span><span/></div>{items.map(item=><button className="project-table-row" key={item.id} onClick={()=>onSelect(item)}><span className="project-main"><i>{item.name.slice(0,1)}</i><span><b>{item.name}</b><small>{item.id} · {item.customer} · {item.source}</small></span></span><span><em className={`stage-tag project-stage-${projectStages.indexOf(item.stage)}`}>{item.stage}</em></span><span className="project-progress"><span><i style={{width:`${item.progress}%`}}/></span><b>{item.progress}%</b></span><span className={`health-label ${item.health}`}>{item.health==="normal"?"正常":item.health==="warning"?"关注":"高风险"}</span><span className="owner-cell"><i>{item.mic.slice(-1)}</i>{item.mic}</span><span className="owner-cell"><i>{item.pic.slice(-1)}</i>{item.pic}</span><span>{item.tasks}</span><span>{item.start.slice(5)} ~ {item.end.slice(5)}</span><span>›</span></button>)}<footer>共 {items.length} 个项目 <span>‹ 1 2 ›</span></footer></section>}
function ProjectBoard({items,onSelect}:{items:Project[];onSelect:(item:Project)=>void}) {return <section className="project-board-grid">{projectStages.map(stage=>{const list=items.filter(item=>item.stage===stage);return <div className="project-board-column" key={stage}><header><div><i className={`stage-dot stage-${projectStages.indexOf(stage)+1}`}/><strong>{stage}</strong><span>{list.length}</span></div><small>¥ {list.reduce((s,i)=>s+i.amount,0)}万</small></header>{list.map(item=><button className="project-board-card" key={item.id} onClick={()=>onSelect(item)}><header><span>{item.id}</span><em className={item.health}>{item.health==="normal"?"正常":item.health==="warning"?"关注":"高风险"}</em></header><strong>{item.name}</strong><small>{item.customer} · {item.source}</small><div className="board-progress"><span><i style={{width:`${item.progress}%`}}/></span><b>{item.progress}%</b></div><p>下一里程碑：{item.milestone}</p><footer><span className="owner-avatar">{item.mic.slice(-1)}</span>MIC {item.mic}<b>PIC {item.pic}</b></footer></button>)}{list.length===0&&<div className="board-empty">暂无项目</div>}</div>})}</section>}
function ProjectDetail({item,onClose,onNotify}:{item:Project;onClose:()=>void;onNotify:(message:string)=>void}) {const [tab,setTab]=useState("概览");return <><button className="drawer-mask" aria-label="关闭项目详情" onClick={onClose}/><aside className="opportunity-drawer project-drawer"><header><div><span>{item.id} · {item.source}</span><h2>{item.name}</h2><p>{item.customer} · MIC {item.mic} · PIC {item.pic}</p></div><button onClick={onClose}>×</button></header><div className="stage-path">{projectStages.map((stage,index)=><div className={index<=projectStages.indexOf(item.stage)?"done":""} key={stage}><i>{index<projectStages.indexOf(item.stage)?"✓":index+1}</i><span>{stage}</span></div>)}</div><div className="drawer-actions"><button onClick={()=>onNotify("已打开项目编辑表单")}>编辑项目</button><button onClick={()=>onNotify("已打开任务拆分入口")}>拆分任务</button><button className="advance" onClick={()=>onNotify(item.stage==="验收结项"?"已发起项目归档":"已模拟推进项目阶段")}>{item.stage==="验收结项"?"项目归档":"推进阶段 →"}</button></div><nav>{["概览","里程碑","项目任务","项目团队","项目资料"].map(label=><button className={tab===label?"active":""} onClick={()=>setTab(label)} key={label}>{label}{label==="项目任务"&&<i>52</i>}{label==="项目团队"&&<i>{item.team}</i>}</button>)}</nav>{tab==="概览"?<ProjectOverview item={item}/>:<ProjectDetailTab tab={tab} item={item} onNotify={onNotify}/>}</aside></>}
function ProjectOverview({item}:{item:Project}) {return <div className="drawer-content"><section><h3>项目概况</h3><dl><div><dt>项目阶段</dt><dd>{item.stage}</dd></div><div><dt>项目健康度</dt><dd><span className={`health-label ${item.health}`}>{item.health==="normal"?"正常":item.health==="warning"?"关注":"高风险"}</span></dd></div><div><dt>合同金额</dt><dd>¥ {item.amount} 万元</dd></div><div><dt>项目来源</dt><dd>{item.source}</dd></div><div><dt>计划周期</dt><dd>{item.start} ~ {item.end}</dd></div><div><dt>MIC</dt><dd>{item.mic}</dd></div><div><dt>PIC</dt><dd>{item.pic}</dd></div></dl></section><section><h3>整体进度</h3><div className="detail-project-progress"><strong>{item.progress}%</strong><span><i style={{width:`${item.progress}%`}}/></span><small>已完成任务 {item.tasks} · 团队 {item.team} 人</small></div></section><section><h3>下一里程碑</h3><div className="detail-next"><i className={item.health}/><div><strong>{item.milestone}</strong><small>计划完成：2026-08-15 · MIC：{item.mic} · PIC：{item.pic}</small></div><button>查看</button></div></section></div>}
function ProjectDetailTab({tab,item,onNotify}:{tab:string;item:Project;onNotify:(message:string)=>void}) {if(tab==="里程碑")return <div className="drawer-content milestone-list"><button onClick={()=>onNotify("已打开新增里程碑表单")}>＋ 新增里程碑</button>{["项目启动会","总体方案评审",item.milestone,"项目终验"].map((name,index)=><article key={name}><i className={index<2?"done":index===2?"doing":""}>{index<2?"✓":index+1}</i><div><strong>{name}</strong><small>{index<2?"已完成":index===2?"进行中 · 08-15":"计划 · 09-30"}</small></div><em>{index<2?"按期":index===2?"剩余 24 天":"未开始"}</em></article>)}</div>;if(tab==="项目任务")return <div className="drawer-content opportunity-task-list"><button onClick={()=>onNotify("已进入任务拆分")}>＋ 拆分任务</button>{["完成接口详细设计","部署测试环境","组织用户验收测试","整理交付材料"].map((name,index)=><article key={name}><i className={index===0?"doing":""}>{index>1?"":"✓"}</i><div><strong>{name}</strong><small>{index%2?"解决方案组":"研发组"} · 08-{12+index}</small></div><em>{index===0?"已完成":index===1?"进行中":"待开始"}</em></article>)}</div>;if(tab==="项目团队")return <div className="drawer-content team-list"><header><strong>项目团队 · {item.team} 人</strong><button onClick={()=>onNotify("已打开添加成员窗口")}>＋ 添加成员</button></header>{[item.mic,item.pic,"周晨","赵倩"].map((name,index)=><article key={`${name}-${index}`}><span>{name.slice(-1)}</span><div><strong>{name}</strong><small>{index===0?"MIC":index===1?"PIC":"实施顾问"}</small></div><em>{index===0?"管理负责人":index===1?"交付负责人":"40% 负载"}</em></article>)}</div>;return <div className="drawer-content opportunity-files"><header><div><strong>项目资料</strong><small>按项目阶段管理过程文档与交付成果</small></div><button onClick={()=>onNotify("已打开资料上传窗口")}>↑ 上传资料</button></header>{projectStages.slice(0,3).map((stage,index)=><section key={stage}><h3>{stage}<span>{3+index}</span></h3><article><span>{index===2?"XLS":"DOC"}</span><div><strong>{stage}_{index===0?"启动材料":index===1?"总体方案":"交付清单"}_V1.{index===2?"xlsx":"docx"}</strong><small>{item.mic} · 2026-07-{12+index} · 3.2 MB</small></div><button onClick={()=>onNotify("已模拟下载项目资料")}>↓</button><button>···</button></article></section>)}</div>}
function ProjectCreate({onClose,onCreated}:{onClose:()=>void;onCreated:()=>void}) {const [source,setSource]=useState("商机转化");return <><button className="modal-mask" aria-label="关闭新建项目" onClick={onClose}/><section className="opportunity-modal project-modal"><header><div><h2>新建项目</h2><p>支持从赢单商机转化，也可以直接录入新项目。</p></div><button onClick={onClose}>×</button></header><div className="source-choice"><button className={source==="商机转化"?"active":""} onClick={()=>setSource("商机转化")}><i>◎</i><div><strong>从商机转化</strong><small>继承客户、合同与商机资料</small></div></button><button className={source==="直接新建"?"active":""} onClick={()=>setSource("直接新建")}><i>＋</i><div><strong>直接新建</strong><small>手动录入完整项目信息</small></div></button></div><div className="form-section"><h3>项目基本信息</h3>{source==="商机转化"&&<label className="wide">来源商机 <b>*</b><select><option>OPP-2026-013 · 统一数据服务平台 · 北辰医药</option><option>OPP-2026-016 · 数据治理一期 · 浦江能源</option></select></label>}<label className="wide">项目名称 <b>*</b><input placeholder="请输入项目名称" defaultValue={source==="商机转化"?"统一数据服务平台":""}/></label><label>客户名称 <b>*</b><select><option>北辰医药</option><option>吉利研究院</option><option>华东制造集团</option></select></label><label>MIC <b>*</b><select><option>王迪</option><option>李娜</option><option>周晨</option></select></label><label>PIC <b>*</b><select><option>李娜</option><option>赵倩</option><option>周晨</option><option>王迪</option></select></label><label>计划开始日期<input type="date"/></label><label>计划结束日期<input type="date"/></label><label>合同金额（万元）<input type="number" placeholder="0.00"/></label><label>初始阶段<select><option>项目启动</option><option>规划设计</option></select></label><label className="wide">项目目标与范围<textarea placeholder="描述项目建设目标、交付范围和关键成功标准"/></label></div><footer><button onClick={onClose}>取消</button><button className="save-draft" onClick={onCreated}>保存草稿</button><button className="primary-action" onClick={onCreated}>创建项目</button></footer></section></>}
