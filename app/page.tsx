...
type Project = { id:string; name:string; customer:string; source:string; stage:string; manager:string; progress:number; health:"normal"|"warning"|"danger"; start:string; end:string; amount:number; team:number; tasks:string; milestone:string };
const projectStages = ["项目启动","规划设计","实施交付","验收结项"];
const projects: Project[] = [
  {id:"PRJ-2026-003",name:"智能研发协同平台",customer:"吉利研究院",source:"商机转化",stage:"实施交付",manager:"王迪",progress:68,health:"normal",start:"2026-04-08",end:"2026-09-30",amount:280,team:8,tasks:"36 / 52",milestone:"用户验收测试"},
  {id:"PRJ-2026-006",name:"供应链流程优化",customer:"恒星零售",source:"商机转化",stage:"验收结项",manager:"李娜",progress:92,health:"warning",start:"2026-02-16",end:"2026-07-31",amount:210,team:5,tasks:"43 / 46",milestone:"项目终验"},
  {id:"PRJ-2026-009",name:"DevSecOps 能力建设",customer:"远海金融",source:"直接新建",stage:"规划设计",manager:"周晨",progress:34,health:"normal",start:"2026-06-01",end:"2026-11-15",amount:165,team:6,tasks:"12 / 38",milestone:"总体方案评审"},
  {id:"PRJ-2026-011",name:"数据治理一期",customer:"浦江能源",source:"商机转化",stage:"项目启动",manager:"赵倩",progress:12,health:"danger",start:"2026-07-01",end:"2026-12-20",amount:420,team:7,tasks:"4 / 34",milestone:"启动会"},
  {id:"PRJ-2026-012",name:"客户数据中台升级",customer:"北辰医药",source:"直接新建",stage:"实施交付",manager:"王迪",progress:57,health:"warning",start:"2026-03-18",end:"2026-10-15",amount:320,team:9,tasks:"28 / 49",milestone:"数据迁移演练"},
];

function ProjectManagement({view,onNavigate}:{view:"list"|"board";onNavigate:(page:PageId)=>void}) {
  const [selected,setSelected]=useState<Project|null>(null); const [creating,setCreating]=useState(false); const [keywordInput,setKeywordInput]=useState(""); const [keyword,setKeyword]=useState(""); const [stageInput,setStageInput]=useState("全部阶段"); const [stageFilter,setStageFilter]=useState("全部阶段"); const [managerInput,setManagerInput]=useState("全部负责人"); const [managerFilter,setManagerFilter]=useState("全部负责人"); const [toast,setToast]=useState("");
  const filtered=projects.filter(item=>(stageFilter==="全部阶段"||item.stage===stageFilter)&&(managerFilter==="全部负责人"||item.manager===managerFilter)&&`${item.name}${item.customer}${item.id}`.includes(keyword));
  const query=()=>{setKeyword(keywordInput.trim());setStageFilter(stageInput);setManagerFilter(managerInput)}; const notify=(message:string)=>{setToast(message);window.setTimeout(()=>setToast(""),2200)};
