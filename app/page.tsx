...
function ProjectTable({items,onSelect}:{items:Project[];onSelect:(item:Project)=>void}) {return <section className="project-table"><div className="project-table-head"><span>项目信息</span><span>阶段</span><span>进度</span><span>健康度</span><span>PIC</span><span>MIC</span><span>任务</span><span>计划周期</span><span/></div>{items.map(item=><button className="project-table-row" key={item.id} onClick={()=>onSelect(item)}><span className="project-main"><i>{item.name.slice(0,1)}</i><span><b>{item.name}</b><small>{item.id} · {item.customer} · {item.source}</small></span></span><span><em className={`stage-tag project-stage-${projectStages.indexOf(item.stage)}`}>{item.stage}</em></span><span className="project-progress"><span><i style={{width:`${item.progress}%`}}/></span><b>{item.progress}%</b></span><span className={`health-label ${item.health}`}>{item.health==="normal"?"正常":item.health==="warning"?"关注":"高风险"}</span><span className="owner-cell"><i>{item.pic.slice(-1)}</i>{item.pic}</span><span className="owner-cell"><i>{item.mic.slice(-1)}</i>{item.mic}</span><span>{item.tasks}</span><span>{item.start.slice(5)} ~ {item.end.slice(5)}</span><span>›</span></button>)}<footer>共 {items.length} 个项目 <span>‹ 1 2 ›</span></footer></section>}
function ProjectBoard({items,onSelect}:{items:Project[];onSelect:(item:Project)=>void}) {return <section className="project-board-grid">{projectStages.map(stage=>{const list=items.filter(item=>item.stage===stage);return <div className="project-board-column" key={stage}><header><div><i className={`stage-dot stage-${projectStages.indexOf(stage)+1}`}/><strong>{stage}</strong><span>{list.length}</span></div><small>¥ {list.reduce((s,i)=>s+i.amount,0)}万</small></header>{list.map(item=><button className="project-board-card" key={item.id} onClick={()=>onSelect(item)}><header><span>{item.id}</span><em className={item.health}>{item.health==="normal"?"正常":item.health==="warning"?"关注":"高风险"}</em></header><strong>{item.name}</strong><small>{item.customer} · {item.source}</small><div className="board-progress"><span><i style={{width:`${item.progress}%`}}/></span><b>{item.progress}%</b></div><p>下一里程碑：{item.milestone}</p><footer><span className="owner-avatar">{item.pic.slice(-1)}</span>PIC {item.pic}<b>MIC {item.mic}</b></footer></button>)}{list.length===0&&<div className="board-empty">暂无项目</div>}</div>})}</section>}
function ProjectDetail({item,onClose,onNotify}:{item:Project;onClose:()=>void;onNotify:(message:string)=>void}) {const [tab,setTab]=useState("概览");return <><button className="drawer-mask" aria-label="关闭项目详情" onClick={onClose}/><aside className="opportunity-drawer project-drawer"><header><div><span>{item.id} · {item.source}</span><h2>{item.name}</h2><p>{item.customer} · PIC {item.pic} · MIC {item.mic}</p></div><button onClick={onClose}>×</button></header><div className="stage-path">{projectStages.map((stage,index)=><div className={index<=projectStages.indexOf(item.stage)?"done":""} key={stage}><i>{index<projectStages.indexOf(item.stage)?"✓":index+1}</i><span>{stage}</span></div>)}</div><div className="drawer-actions"><button onClick={()=>onNotify("已打开项目编辑表单")}>编辑项目</button><button onClick={()=>onNotify("已打开任务拆分入口")}>拆分任务</button><button className="advance" onClick={()=>onNotify(item.stage==="验收结项"?"已发起项目归档":"已模拟推进项目阶段")}>{item.stage==="验收结项"?"项目归档":"推进阶段 →"}</button></div><nav>{["概览","里程碑","项目任务","项目团队","项目资料"].map(label=><button className={tab===label?"active":""} onClick={()=>setTab(label)} key={label}>{label}{label==="项目任务"&&<i>52</i>}{label==="项目团队"&&<i>{item.team}</i>}</button>)}</nav>{tab==="概览"?<ProjectOverview item={item}/>:<ProjectDetailTab tab={tab} item={item} onNotify={onNotify}/>}</aside></>}
function ProjectOverview({item}:{item:Project}) {return <div className="drawer-content"><section><h3>项目概况</h3><dl><div><dt>项目阶段</dt><dd>{item.stage}</dd></div><div><dt>项目健康度</dt><dd><span className={`health-label ${item.health}`}>{item.health==="normal"?"正常":item.health==="warning"?"关注":"高风险"}</span></dd></div><div><dt>合同金额</dt><dd>¥ {item.amount} 万元</dd></div><div><dt>项目来源</dt><dd>{item.source}</dd></div><div><dt>计划周期</dt><dd>{item.start} ~ {item.end}</dd></div><div><dt>PIC</dt><dd>{item.pic}</dd></div><div><dt>MIC</dt><dd>{item.mic}</dd></div></dl></section><section><h3>整体进度</h3><div className="detail-project-progress"><strong>{item.progress}%</strong><span><i style={{width:`${item.progress}%`}}/></span><small>已完成任务 {item.tasks} · 团队 {item.team} 人</small></div></section><section><h3>下一里程碑</h3><div className="detail-next"><i className={item.health}/><div><strong>{item.milestone}</strong><small>计划完成：2026-08-15 · PIC：{item.pic} · MIC：{item.mic}</small></div><button>查看</button></div></section></div>}
